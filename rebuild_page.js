...
const pageLines = fs.readFileSync('src/app/event/[id]/page.tsx', 'utf8').split('\n');
const originalReturnLines = fs.readFileSync('original_return.txt', 'utf8').split('\n');

const pageHead = pageLines.slice(0, 255).join('\n');

const loadingAndNotEvent = originalReturnLines.slice(0, 22).join('\n');

const participantViewInner = originalReturnLines.slice(24, 580).join('\n');
const participantView = `
  if (isParticipant) {
    return (
      <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-20">
${participantViewInner}
      </div>
    );
  }
`;
...
const finalCode = [
  pageHead,
  '  if (loading) {', // Add the opening if back
  loadingAndNotEvent,
  participantView,
  newJSX
].join('\n');

fs.writeFileSync('src/app/event/[id]/page.tsx', finalCode);
console.log('Rebuilt page.tsx');
